...
async def update_data_item(
    payload: DataPayload,
    item_id: str = Path(..., description="The ID of the data item to update")
):
    """Update data item fields"""
    updated = await lightctrl.update_data_item(item_id, payload.dict())
    if updated["status"] == "not found":
        raise HTTPException(status_code=404, detail=f"Data item '{item_id}' not found")
    if "item" not in updated:
        raise HTTPException(status_code=400, detail=f"Update failed: {updated.get('status', 'unknown error')}")
    return updated["item"]
